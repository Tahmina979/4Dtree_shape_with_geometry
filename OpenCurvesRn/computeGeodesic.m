...
q1 = curve_to_tq(M1,doNormalizeScale);

q2 = curve_to_tq(M2,doNormalizeScale);
...
for i=1:nSamples
    ...
    refc=(1-t)*M1 + t*M2;
    if i==1
         Xgeod(i, :, :) =q_to_curveo(q1,M,refc);
    elseif i==nSamples
          Xgeod(i, :, :) =q_to_curveo(q2,M,refc);
    else
          Xgeod(i, :, :)= q_to_curveo(q,M,refc);%(1-t)*M1 + t*M2;
