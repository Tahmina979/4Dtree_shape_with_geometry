...
for k=1: numel(Q.q_children)
    ...
    absoluteDifferences = abs(Q.t_paras - Q.tk_sideLocs(k));
    [minDifference, nearestIndex] = min(absoluteDifferences);
    check_value=beta0_rad(nearestIndex);
    
    if Q.q_children{k}.beta0_rad(1,1)>check_value
        Q.beta{k} = repmat(check_value,1,size(Q.q_children{k}.beta0_rad,2));
        Q.q_children{k}.beta0_rad=Q.beta{k};
    else
